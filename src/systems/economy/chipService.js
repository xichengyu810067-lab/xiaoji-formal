...
function mapChipAccount(row, sourceGuildId = null) {
  return {
...
function ensureChipAccount(api, guildId, userId, timestamp = nowIso()) {
  ...
  api.run(
    `INSERT INTO chip_accounts_global (user_id, balance, created_at, updated_at)
     VALUES (?, 0, ?, ?)
     ON CONFLICT(user_id) DO NOTHING`,
    [userId, timestamp, timestamp]
  );

  return mapChipAccount(api.get('SELECT * FROM chip_accounts_global WHERE user_id = ?', [userId]), guildId);
}

function writeChipBalance(api, guildId, userId, balance, timestamp = nowIso()) {
  api.run(
    'UPDATE chip_accounts_global SET balance = ?, updated_at = ? WHERE user_id = ?',
    [balance, timestamp, userId]
  );
}
