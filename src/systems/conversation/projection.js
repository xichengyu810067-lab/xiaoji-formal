...
function deleteMemoryProjection({ userId, guildId = null, scope = guildId ? 'guild' : 'all',
  channelId = null, from = null, to = null }) {
  if (!userId) throw new Error('userId is required.');
  if (!fs.existsSync(getMemoryPath())) return { persisted: true, absent: true };
  const loaded = readMemoryState();
  if (!loaded.writable) return { persisted: false, reason: 'corrupt_or_unavailable' };
  if (!loaded.memory.private_user_memory || typeof loaded.memory.private_user_memory !== 'object' ||
    !loaded.memory.public_channel_memory || typeof loaded.memory.public_channel_memory !== 'object' ||
    [...Object.values(loaded.memory.private_user_memory),
      ...Object.values(loaded.memory.public_channel_memory)].some((records) => !Array.isArray(records) ||
        records.some((record) => !record || typeof record !== 'object'))) {
    return { persisted: false, reason: 'invalid_projection_shape' };
  }
  if ((loaded.memory.private_user_memory[userId] || []).some((record) =>
    record.userId && record.userId !== userId)) {
    return { persisted: false, reason: 'private_projection_identity_conflict' };
  }
  const ownedRecords = [
    ...(loaded.memory.private_user_memory[userId] || []),
    ...Object.values(loaded.memory.public_channel_memory).flat().filter((record) => record.userId === userId),
  ];
  if ((scope !== 'all' && ownedRecords.some((record) => record.guildId === undefined)) ||
    (channelId !== null && ownedRecords.some((record) => record.channelId === undefined))) {
    return { persisted: false, reason: 'unknown_projection_scope' };
  }
  const matchesIdentityScope = (record, ownerHint = null) =>
    (record.userId === userId || ownerHint === userId && !record.userId) &&
    (scope !== 'dm' || record.guildId === null) &&
    (scope !== 'guild' || record.guildId === guildId) &&
    (channelId === null || record.channelId === channelId);
  const hasTimeRange = from !== null || to !== null;
  const memory = loaded.memory;
  const targetRecords = [
    ...Object.entries(memory.private_user_memory).flatMap(([owner, records]) =>
      records.filter((record) => matchesIdentityScope(record, owner))),
    ...Object.values(memory.public_channel_memory).flat().filter((record) => matchesIdentityScope(record)),
  ];
  if (hasTimeRange && targetRecords.some((record) => !Number.isFinite(Date.parse(record.timestamp)))) {
    return { persisted: false, reason: 'unknown_target_timestamp' };
  }
  const belongsToScope = (record, ownerHint = null) => matchesIdentityScope(record, ownerHint) &&
    (!hasTimeRange || Number.isFinite(Date.parse(record.timestamp))) &&
    (from === null || Date.parse(record.timestamp) >= Date.parse(from)) &&
    (to === null || Date.parse(record.timestamp) <= Date.parse(to));
  for (const [owner, records] of Object.entries(memory.private_user_memory)) {
    if (owner === userId && Array.isArray(records)) {
      memory.private_user_memory[owner] = records.filter((record) => !belongsToScope(record, owner));
    }
  }
  for (const [guild, records] of Object.entries(memory.public_channel_memory)) {
    if (Array.isArray(records)) memory.public_channel_memory[guild] = records.filter((record) => !belongsToScope(record));
  }
  try {
    writeMemory(memory);
    recentPublicMessages.clear();
